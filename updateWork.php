...
if (isset($data["username"])) {
	...
	$work = mysqli_real_escape_string($conn,$data['work']);
	$userId = mysqli_real_escape_string($conn,$data['userId']);
	
	if (validateUser($username,$userId)) {
		$q = mysqli_query($conn, "update users set work='$work' where username='$username' and id='$userId'");
		if($q){
			$obj = new StdClass();
			$obj->msg = "Firstname updated successfull";
			$obj->type = "success";
